...
<p align="center">
  ...
  Remaps CapsLock to switch keyboard layout via a low-level hook.<br/>
  No config, no tray icon, no dependencies. Just runs and stays out of your way.
</p>
...
Hooks `WH_KEYBOARD_LL` globally.

**Two modes:**

| Mode | Behavior |
|------|----------|
| **Long-press** (default, 300ms) | Tap CapsLock → language switch. Hold CapsLock (300ms) → real CapsLock toggle. |
| **Legacy** (`0` as arg) | CapsLock always switches language. Hold **Shift + CapsLock** for normal CapsLock toggle (passthrough). |
...
```
CapsLang-x64.exe [timeout_ms] [method]
```

| Arg | Values | Default | Description |
|-----|--------|---------|-------------|
| `timeout_ms` | number ≥ 0 | `300` | Long-press threshold. `0` = legacy (no long-press) |
| `method` | `wm` or `win+space` | `wm` | Language switch method |

**Examples:**

```powershell
CapsLang-x64.exe                    # WM message, 300ms
CapsLang-x64.exe 500                # WM message, 500ms
CapsLang-x64.exe 300 win+space      # Win+Space, 300ms
CapsLang-x64.exe 0                  # legacy (immediate Win+Space, no long-press)
```

## Download

Pre-built AOT-published executables are available in [Releases](https://github.com/frytech/CapsLang/releases).
...
4. **Actions** tab: **New** → *Action:* `Start a program` → *Program:* full path to `CapsLang.exe` → *Add arguments (optional):* e.g. `500 wm` or `300 win+space`
...
$arguments = ""                    # WM message, 300ms
# $arguments = "500"               # WM message, 500ms
# $arguments = "300 win+space"     # Win+Space, 300ms
# $arguments = "0"                 # legacy mode (immediate, no long-press)
